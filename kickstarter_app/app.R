...
# Import data
kickstarter <- read_csv("kickstarter_data.csv")
...
kickstarter$country_display_name_corrected <- as.character(kickstarter$country_display_name_corrected) 
...
category_choices <- c("All", unique(sort(kickstarter$main_category)))
country_choices <- c("All", unique(sort(kickstarter$country_display_name_corrected)))
outcome_choices <- c("All", unique(sort(kickstarter$outcome)))
  ...
  data_for_table <- reactive({
    ...
    data <- kickstarter %>%
      select(name, backers_count, pledged_usd, goal_usd, pct_funded, outcome, 
             main_category, sub_category, launched_at, campaign_length, country_display_name_corrected, project_urls) %>%
      ...
      filter(main_category %in% category_input, # filtering by various inputs
             ...
             country_display_name_corrected %in% country_input,
      ...
      rename("Name" = name, "Backers" = backers_count, "Raised ($)" = pledged_usd, "Goal ($)" = goal_usd,
             ...
             "Subcategory" = sub_category, "Launch Date" = launched_at, "Campaign Length (days)" = campaign_length, "Country" = country_display_name_corrected,
             "URL" = project_urls)
  })
